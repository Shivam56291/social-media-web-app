...
export default function PostCard({ post, onShareClick }) {
  ...
  // States for Lightbox (Handles both normal and nested images)
  const [lightboxOpen, setLightboxOpen] = useState(false);
  const [lightboxIndex, setLightboxIndex] = useState(0);
  const [lightboxSource, setLightboxSource] = useState("normal"); // "normal" or "parent"
  ...
  // Determine which image array the lightbox should read from
  const currentLightboxImages = lightboxSource === "parent" ? parentImages : images;
  ...
  // Lightbox handlers supporting sources ('normal' | 'parent')
  const openLightbox = (index, source = "normal") => {
  ...
  const nextLightboxImage = (e) => {
    e.stopPropagation();
    if (lightboxIndex < currentLightboxImages.length - 1) {
    ...
    <>
        ...
        <div className="flex items-center justify-between p-5">
          <div className="flex items-center gap-4">
            ...
            <div
              ...
              className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-500 to-cyan-500 font-bold"
            >
              {user?.avatar_url ? (
          ...
          <div ref={menuRef} className="relative">
            ...
            >
              <DotsThree size={24} className="text-slate-400" />
            </button>
            ...
            <AnimatePresence>
              {showMenu && post.author === currentUser?.id && (
                <motion.div
                  ...
                  className="absolute right-0 top-12 z-30 w-52 overflow-hidden rounded-2xl border border-white/10 bg-[#0f172a]/95 shadow-2xl backdrop-blur-xl"
                >
                  <button
                    onClick={() => {
                      setOpenEdit(true);
                    ...
                    className="flex w-full items-center gap-3 px-4 py-3 text-sm text-slate-200 transition-all hover:bg-cyan-500/10"
                  >
                  ...
                  {parentImages.length === 1 ? (
                    ...
                    <div className="grid grid-cols-2 gap-1.5 aspect-video w-full max-h-80">
                      {parentImages.slice(0, 4).map((img, idx) => (
                        <div
                          key={idx}
                          onClick={() => openLightbox(idx, "parent")}
              ...
              <div className="relative h-[420px] w-full md:h-[520px]">
                <AnimatePresence mode="wait">
                  <motion.img
                    ...
                    alt="post"
                    loading="lazy"
                    onClick={() => openLightbox(activeImage, "normal")}
                    initial={{ opacity: 0, x: 40 }}
                    animate={{ opacity: 1, x: 0 }}
                    exit={{ opacity: 0, x: -40 }}
                    transition={{ duration: 0.25 }}
                    className="absolute inset-0 h-full w-full object-cover cursor-pointer hover:scale-[1.01] transition-transform duration-300"
                  />
                </AnimatePresence>

                {/* COUNT */}
                {images.length > 1 && (
                  <div className="absolute right-4 top-4 rounded-full bg-black/50 px-3 py-1 text-xs backdrop-blur-xl">
                ...
                {activeImage > 0 && (
                  <button
                    onClick={prevImage}
                    className="absolute left-4 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/40 backdrop-blur-xl"
                  >
                    <CaretLeft size={22} />
                  </button>
                )}
                ...
                {activeImage < images.length - 1 && (
                  <button
                    onClick={nextImage}
                    className="absolute right-4 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/40 backdrop-blur-xl"
                  >
                    <CaretRight size={22} />
                  </button>
                )}
                ...
                {images.map((image, index) => (
                  <button
                    ...
                    className={`relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl border transition-all ${
                      activeImage === index ? "border-cyan-400" : "border-white/10"
                    }`}
                  >
      ...
      <AnimatePresence>
        {showComments && (
          ...
          >
            <CommentsPreview post={post} onOpenModal={() => setCommentsModalOpen(true)} />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <CommentsModal open={commentsModalOpen} onClose={() => setCommentsModalOpen(false)} post={post} />

      {/* DYNAMIC LIGHTBOX FOR BOTH NORMAL AND NESTED IMAGES */}
      <AnimatePresence>
        {lightboxOpen && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            <div className="relative w-full max-w-5xl aspect-video max-h-[85vh] overflow-hidden rounded-3xl border border-white/5 bg-neutral-950 flex items-center justify-center">
              ...
              {lightboxIndex < currentLightboxImages.length - 1 && (
                <button
